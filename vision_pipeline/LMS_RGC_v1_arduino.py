...
def run(camera_index: int = CAMERA_INDEX) -> np.ndarray:
    ...
    arduino = serial.Serial('/dev/cu.usbmodem14401', 9600, timeout=1)  # update port if it shifts again
    time.sleep(2)  # allow Arduino to reset after connecting
    PIXEL_ROW, PIXEL_COL, CHANNEL = 16, 16, 0  # center pixel, L cone — change as needed
    ...
    while True:
        ...
        pixel_mv = grid[PIXEL_ROW, PIXEL_COL, CHANNEL]
        scaled_voltage = (pixel_mv / V_MAX) * 2.0  # map 0-30mV to 0-2V
        arduino.write(f"{scaled_voltage:.2f}\n".encode())
        time.sleep(0.05)   # <--potentially adjust this to control how fast values stream to Arduino; as the measured voltage remained flat while the camera was moving (capturing different voltages)
